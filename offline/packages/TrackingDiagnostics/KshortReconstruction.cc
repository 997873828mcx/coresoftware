...
int KshortReconstruction::InitRun(PHCompositeNode* topNode)
{
    ...
    // Initialize the testParticle with some parameters
    float position[3] = {0.0, 0.0, 0.0};
    float momentum[3] = {0.0, 0.0, 1.0}; // Particle moving along z-axis

    // Combine position and momentum into a single Param array
    float param[6] = {
    ...
    float cov[21] = {0}; // Initialize all elements to zero
    cov[0] = cov[2] = cov[5] = 1e-4f;   // Position variances
    cov[9] = cov[14] = cov[20] = 1e-4f; // Momentum variances

    int charge = 1; // Positive charge
    float mass = 0.13957039f; // Pion mass in GeV/c^2

    // Call the Create function with the correct parameters
    testParticle.Create(param, cov, charge, mass);

    // Call a method that exists in KFParticle
    float particleMass = testParticle.GetMass();

    // Print out the mass to verify
    std::cout << "Test KFParticle created with mass: " << particleMass << " GeV/c^2" << std::endl;
...
int KshortReconstruction::process_event(PHCompositeNode* /**topNode*/)
{
    ...
    for (auto tr1_it = m_svtxTrackMap->begin(); tr1_it != m_svtxTrackMap->end(); ++tr1_it)
    {
        ...
        // first dca cuts
        if (apply_dca_cut && (dcaVals1(0) < this_dca_cut || dcaVals1(1) < this_dca_cut))
        ...
        for (auto tr2_it = std::next(tr1_it); tr2_it != m_svtxTrackMap->end(); ++tr2_it)
        {
            ...
            if (apply_dca_cut && (dcaVals2(0) < this_dca_cut2 || dcaVals2(1) < this_dca_cut2))
...
void KshortReconstruction::fillNtp(SvtxTrack* track1, SvtxTrack* track2, Acts::Vector3 dcavals1, Acts::Vector3 dcavals2, Acts::Vector3 pca_rel1, Acts::Vector3 pca_rel2, double pair_dca, double invariantMass, double invariantPt, float rapidity, float pseudorapidity, Eigen::Vector3d projected_pos1, Eigen::Vector3d projected_pos2, Eigen::Vector3d projected_mom1, Eigen::Vector3d projected_mom2, Acts::Vector3 pca_rel1_proj, Acts::Vector3 pca_rel2_proj, double pair_dca_proj, unsigned int track1_silicon_cluster_size, unsigned int track2_silicon_cluster_size)
{
    ...
    if (!svtxVertex)
    {
        ...
        vertex = Acts::Vector3(-0.11, 2.756, -0.526);  // primary vertex
        //return;
    }else{
...
void KshortReconstruction::fillHistogram(Eigen::Vector3d mom1, Eigen::Vector3d mom2, TH1D* massreco, double& invariantMass, double& invariantPt, float& rapidity, float& pseudorapidity)
{
    ...
    if (!apply_invariant_pt_cut || invariantPt > invariant_pt_cut)
    {
