...
int KshortReconstruction::process_event(PHCompositeNode* topNode)
{
  ...
  for (auto tr1_it = m_svtxTrackMap->begin(); tr1_it != m_svtxTrackMap->end(); ++tr1_it)
  {
    ...
    if (apply_track_pt_cut && tr1->get_pt() < track_pt_cut)
    {
      continue;
    }
    ...
    // first dca cuts
    if (apply_dca_cut && (fabs(dcaVals1(0)) < this_dca_cut || fabs(dcaVals1(1)) < this_dca_cut))
    ...
    for (auto tr2_it = std::next(tr1_it); tr2_it != m_svtxTrackMap->end(); ++tr2_it)
    {
      ...
      if (apply_quality_cut && tr2->get_quality() > _qual_cut)
      {
        continue;
      }

      if (apply_track_pt_cut && tr2->get_pt() < track_pt_cut)
      {
        continue;
      }
      ...
      if (apply_dca_cut && (fabs(dcaVals2(0)) < this_dca_cut2 || fabs(dcaVals2(1)) < this_dca_cut2))
      ...
      // --- inside the outer track loop -----------------------------
      if (apply_charge_cut && tr1->get_charge() == 0)  // skip “neutral” tracks
        continue;

      // --- inside the inner track loop ------------------------------
      if (apply_charge_cut && tr2->get_charge() == 0)
        continue;
      if (apply_charge_cut && tr1->get_charge() == tr2->get_charge())
      ...
      Acts::Vector3 vertex(0, 0, tr1->get_z());  // fake primary vertex
      ...
      TVector3 v0P = TVector3(projected_mom1(0), projected_mom1(1), projected_mom1(2)) + TVector3(projected_mom2(0), projected_mom2(1), projected_mom2(2));
      ...
      if (fabs(denom) < 1e-6) continue;  // skip pathological case
      double alpha = (pL_plus - pL_minus) / denom;
      ...
      double pt1 = std::sqrt(projected_mom1(0) * projected_mom1(0) + projected_mom1(1) * projected_mom1(1));
      double pt2 = std::sqrt(projected_mom2(0) * projected_mom2(0) + projected_mom2(1) * projected_mom2(1));
      if (pt1 < 0.05 || pt2 < 0.05) continue;  // ptcut
      if (cos_theta_reco < 0.9) continue;     // DIRAcut
      float Lproj = static_cast<float>(pathLength_proj.norm());
      if (Lproj < 0.05) continue;                              // proj_cut
      if (std::abs(pair_dca_proj) > 0.3) continue;            // pair_cut
      if (tr1->get_charge() == tr2->get_charge()) continue;    // chargecut
      if (dcaVals1(0) < 0.01 || dcaVals2(0) < 0.01) continue;  // trackdcacut
      m_pair = {};
      m_pair.cross1 = crossing1;
      m_pair.cross2 = crossing2;

      m_pair.run = m_runNumber;
      m_pair.evt = m_evtNumber;

      m_pair.px1 = projected_mom1(0);
      m_pair.py1 = projected_mom1(1);
      m_pair.pz1 = projected_mom1(2);
      m_pair.px2 = projected_mom2(0);
      m_pair.py2 = projected_mom2(1);
      m_pair.pz2 = projected_mom2(2);
      m_pair.dca_xy1 = dcaVals1(0);
      m_pair.dca_z1 = dcaVals1(1);
      m_pair.dca_xy2 = dcaVals2(0);
      m_pair.dca_z2 = dcaVals2(1);
      m_pair.pairDCA = pair_dca_proj;  // or whichever DCA you choose
      m_pair.alpha = alpha;
      m_pair.qT = qT;
      // m_pair.invMass = invariantMass;
      // m_pair.invPt = invariantPt;
      // m_pair.invPhi = invariantPhi;
      //  m_pair.rap = rapidity;
      //  m_pair.eta = pseudorapidity;

      m_pair.charge1 = static_cast<float>(tr1->get_charge());
      m_pair.charge2 = static_cast<float>(tr2->get_charge());

      m_pair.cosThetaReco = cos_theta_reco;

      m_pair.Lproj = static_cast<float>(pathLength_proj.norm());  // in cm
...
int KshortReconstruction::InitRun(PHCompositeNode* topNode)
{
  /* h_AP_all = new TH2D("h_AP_all",
                      ...
                      200, -1.2, 1.2,  // α axis
                      200, 0.0, 0.3);  // qT axis  (≈ pT of + daughter) */

  const char* cfilepath = filepath.c_str();
  fout = new TFile(cfilepath, "recreate");

  m_pairTree = new TTree("pairTree", "all V0 candidates");
  m_pairTree->Branch("run", &m_pair.run, "run/I");
  m_pairTree->Branch("evt", &m_pair.evt, "evt/I");
  m_pairTree->Branch("cross1", &m_pair.cross1, "cross1/S");
  m_pairTree->Branch("cross2", &m_pair.cross2, "cross2/S");

  m_pairTree->Branch("px1", &m_pair.px1, "px1/F");
  m_pairTree->Branch("py1", &m_pair.py1, "py1/F");
  m_pairTree->Branch("pz1", &m_pair.pz1, "pz1/F");
  m_pairTree->Branch("px2", &m_pair.px2, "px2/F");
  m_pairTree->Branch("py2", &m_pair.py2, "py2/F");
  m_pairTree->Branch("pz2", &m_pair.pz2, "pz2/F");
  m_pairTree->Branch("dca_xy1", &m_pair.dca_xy1, "dca_xy1/F");
  m_pairTree->Branch("dca_z1", &m_pair.dca_z1, "dca_z1/F");
  m_pairTree->Branch("dca_xy2", &m_pair.dca_xy2, "dca_xy2/F");
  m_pairTree->Branch("dca_z2", &m_pair.dca_z2, "dca_z2/F");
  m_pairTree->Branch("pairDCA", &m_pair.pairDCA, "pairDCA/F");
  m_pairTree->Branch("alpha", &m_pair.alpha, "alpha/F");
  m_pairTree->Branch("qT", &m_pair.qT, "qT/F");
  // m_pairTree->Branch("invMass", &m_pair.invMass, "invMass/F");
  // m_pairTree->Branch("invPt", &m_pair.invPt, "invPt/F");
  // m_pairTree->Branch("invPhi", &m_pair.invPhi, "invPhi/F");

  m_pairTree->Branch("charge1", &m_pair.charge1, "charge1/F");
  m_pairTree->Branch("charge2", &m_pair.charge2, "charge2/F");
  m_pairTree->Branch("cosThetaReco", &m_pair.cosThetaReco, "cosThetaReco/F");
  m_pairTree->Branch("Lproj", &m_pair.Lproj, "Lproj/F");
